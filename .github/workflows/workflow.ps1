...
$GitHubNugetPackagelist = Invoke-RestMethod -Uri "https://api.github.com/users/carsten-riedel/packages/nuget/Coree.DotnetTool.ToolKit/versions" -Headers $headers
...
{
    $PackageId = $item.id
    Invoke-RestMethod -Method Delete -Uri "https://api.github.com/users/carsten-riedel/packages/nuget/Coree.DotnetTool.ToolKit/versions/$PackageId" -Headers $headers | Out-Null
    Write-Output "Unlisted package Coree.DotnetTool.ToolKit $($item.name)"
}
...
$NugetPackageList = (Invoke-RestMethod -Uri "$NugetRegistrationsBaseUrlAPI$("Coree.DotnetTool.ToolKit".ToLowerInvariant())/index.json").items.items.catalogEntry
...
foreach ($item in $ListedIgnoreNewest)
{
    ...
    Invoke-RestMethod -Uri "https://www.nuget.org/api/v2/package/Coree.DotnetTool.ToolKit/$($item.version)" -Method Delete -Headers $headers | Out-Null
    Write-Output "Unlisted package Coree.DotnetTool.ToolKit $($item.version)"
}
